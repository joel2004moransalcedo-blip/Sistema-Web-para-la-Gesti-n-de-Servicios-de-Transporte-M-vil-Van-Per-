...
<%
    ...
    if (reserva == null) {
        out.println("<div class='container py-5'><div class='alert alert-danger text-center'>⚠️ No se encontró la información de la reserva.</div></div>");
        return;
    }

    int idReserva = ((Number) reserva.get("id_reserva")).intValue();
    double total = reserva.get("total") != null ? ((Number) reserva.get("total")).doubleValue() : 0.0;
    String paqueteNombre = (String) reserva.getOrDefault("paquete_nombre", "Paquete desconocido");
    String fechaReserva = (String) reserva.getOrDefault("fecha_reserva", "");
    String estado = (String) reserva.getOrDefault("estado", "pendiente");
    String paqueteImagen = (String) reserva.getOrDefault("paquete_imagen1", "");
%>
        ...
        <div class="col-lg-7 col-md-10">
            ...
            <div class="card p-4">
                ...
                <div class="row g-3 mb-4 align-items-center">
                    ...
                    <div class="col-md-8">
                        <div class="resumen">
                            <h5 class="mb-1"><strong><%= paqueteNombre %></strong></h5>

                            <p class="mb-1">
                                <i class="bi bi-calendar3"></i>
                                <strong><fmt:message key="payment.reserve.date" />:</strong> <%= fechaReserva %>
                            </p>

                            <p class="mb-1">
                                <i class="bi bi-info-circle"></i>
                                <strong><fmt:message key="payment.reserve.status" />:</strong> <%= estado %>
                            </p>

                            <p class="mb-0 fs-5 text-primary">
                                <strong><fmt:message key="payment.total" />:</strong>
                                S/ <%= String.format(Locale.ENGLISH, "%.2f", total) %>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form id="formPago"
                    ...
                    <input type="hidden" name="monto" value="<%= String.format(Locale.ENGLISH, "%.2f", total) %>">
